# Document Portal - Project Flow Documentation

## Entry Point

The entry point is `api/main.py`, a **FastAPI** application. The server is started using:
```bash
uvicorn api.main:app --host 0.0.0.0 --port 8080 --reload
```

The application provides 5 main endpoints:
1. `GET /` - Serves the HTML UI
2. `GET /health` - Health check
3. `POST /analyze` - Document analysis (metadata extraction)
4. `POST /compare` - Document comparison (PDF diff)
5. `POST /chat/index` - Build FAISS vector index from uploaded documents
6. `POST /chat/query` - Query documents using Conversational RAG
...
```
                    ...
                    ▼               ▼               ▼               ▼               ▼
            ┌───────────┐   ┌───────────┐   ┌───────────┐   ┌───────────┐   ┌───────────┐
            │   GET /   │   │  /health  │   │ /analyze  │   │ /compare  │   │  /chat/*  │
            │(Serve UI) │   │           │   │           │   │           │   │           │
            └───────────┘   └───────────┘   └─────┬─────┘   └─────┬─────┘   └─────┬─────┘
                                                  │               │               │
                                                  ▼               ▼               ▼
                                          ┌───────────┐   ┌───────────┐   ┌───────────────┐
                                          │DocHandler │   │Document   │   │ChatIngestor   │
                                          │+ Analyzer │   │Comparator │   │Conversational │
                                          └─────┬─────┘   │+ LLM      │   │RAG            │
                                                │         └─────┬─────┘   └───────┬───────┘
                                                │               │                 │
                                                ▼               ▼                 ▼
                                          ...
                                          │  (ModelLoader: Embeddings + LLM loading)   │
                                          └─────────────────────┬─────────────────────┘
                                                                │
                                          ┌─────────────────────┼─────────────────────┐
                                          ▼                     ▼                     ▼
                                    ┌───────────┐         ┌───────────┐         ┌───────────┐
                                    │  Google   │         │HuggingFace│         │GoogleGenAI│
                                    │Embeddings │         │Embeddings │         │/ Groq LLM │
                                    └───────────┘         └───────────┘         └───────────┘
```
...
### 1. FastAPI Application Entry (`api/main.py`)

- **Location**: `api/main.py`
- **Purpose**: Main entry point - routes HTTP requests to appropriate handlers
- **Inputs**: HTTP requests (files, form data, query parameters)
- **Output**: JSON responses, HTML responses
- **Logic**: Initializes FastAPI with CORS, static files, and Jinja2 templates. Defines endpoints for document operations.
- **Calls**: `DocHandler`, `DocumentAnalyzer`, `DocumentComparator`, `DocumentComparatorLLM`, `ChatIngestor`, `ConversationalRAG`

---

### 2. `/analyze` Endpoint Flow

#### 2.1 DocHandler

- **Location**: `src/document_ingestion/data_ingestion.py`
- **Purpose**: Handles PDF file operations (save & read) for document analysis
- **Inputs**: 
  - `data_dir: Optional[str]` - Base directory for storage
  - `session_id: Optional[str]` - Session identifier
- **Output**: Instance with `session_id`, `session_path` attributes
- **Logic**: Creates session-based directories, provides `save_pdf()` and `read_pdf()` methods using PyMuPDF (fitz)
- **Calls**: `generate_session_id()`, `fitz.open()`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `save_pdf(uploaded_file)` | File object with `.name` | `str` (saved path) | Saves PDF to session directory |
| `read_pdf(pdf_path: str)` | Path string | `str` (text content) | Extracts text page-by-page with markers |

#### 2.2 DocumentAnalyzer

- **Location**: `src/document_analyzer/data_analysis.py`
- **Purpose**: Extracts structured metadata from document text using LLM
- **Inputs**: None (uses ModelLoader internally)
- **Output**: Instance with `llm`, `parser`, `fixing_parser`, `prompt`
- **Logic**: Initializes LLM via ModelLoader, sets up JsonOutputParser with Pydantic model `Metadata`, creates LangChain chain
- **Calls**: `ModelLoader.load_llm()`, `JsonOutputParser`, `OutputFixingParser`, `PROMPT_REGISTRY`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `analyze_document(document_text: str)` | Document text | `dict` (metadata) | Invokes LLM chain to extract: Title, Author, DateCreated, Summary, etc. |

---

### 3. `/compare` Endpoint Flow

#### 3.1 DocumentComparator (File Handler)

- **Location**: `src/document_ingestion/data_ingestion.py`
- **Purpose**: Saves and combines reference/actual PDFs for comparison
- **Inputs**: 
  - `base_dir: str` - Base directory (default: `data/document_compare`)
  - `session_id: Optional[str]`
- **Output**: Instance with `session_id`, `session_path`
- **Logic**: Creates session directory, provides methods to save paired files and combine their text
- **Calls**: `generate_session_id()`, `fitz.open()`
...
| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `save_uploaded_files(ref, act)` | Two file objects | `tuple[Path, Path]` | Saves both PDFs |
| `combine_documents()` | None | `str` | Combines all PDFs in session with labels |
| `clean_old_sessions(keep_latest)` | `int` | None | Removes old session folders |

#### 3.2 DocumentComparatorLLM

- **Location**: `src/document_compare/document_comparator.py`
- **Purpose**: Compares documents using LLM and returns structured diff as DataFrame
- **Inputs**: None
- **Output**: Instance with LLM chain configured for comparison
- **Logic**: Creates chain: `prompt | llm | JsonOutputParser(SummaryResponse)`. Parses LLM output into DataFrame with Page/Changes columns.
- **Calls**: `ModelLoader.load_llm()`, `PROMPT_REGISTRY["document_comparison"]`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `compare_documents(combined_docs: str)` | Combined text | `pd.DataFrame` | Returns page-wise comparison results |
| `_format_response(response_parsed)` | `list[dict]` | `pd.DataFrame` | Converts LLM response to DataFrame |

---

### 4. `/chat/index` Endpoint Flow

#### 4.1 ChatIngestor

- **Location**: `src/document_ingestion/data_ingestion.py`
- **Purpose**: Main ingestion pipeline - uploads files, chunks documents, builds FAISS index
- **Inputs**:
  - `temp_base: str` - Temp file directory (default: `data`)
  - `faiss_base: str` - FAISS index directory (default: `faiss_index`)
  - `use_session_dirs: bool` - Whether to create session subdirectories
  - `session_id: Optional[str]`
- **Output**: Instance with `session_id`, `temp_dir`, `faiss_dir`
- **Logic**: Manages file storage and FAISS index creation per session
- **Calls**: `ModelLoader()`, `generate_session_id()`, `save_uploaded_files()`, `load_documents()`, `FaissManager`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `built_retriver(files, chunk_size, chunk_overlap, k)` | Iterable files, chunking params | `Retriever` | Full pipeline: save → load → chunk → index → retriever |
| `_split(docs, chunk_size, chunk_overlap)` | `List[Document]` | `List[Document]` | Splits docs using `RecursiveCharacterTextSplitter` |

#### 4.2 FaissManager

- **Location**: `src/document_ingestion/data_ingestion.py`
- **Purpose**: FAISS vector index manager with load-or-create pattern and deduplication
- **Inputs**:
  - `index_dir: Path` - Directory for FAISS files
  - `model_loader: Optional[ModelLoader]`
- **Output**: Instance managing FAISS vectorstore
- **Logic**: Creates/loads FAISS index, tracks ingested docs via fingerprinting to avoid duplicates
- **Calls**: `ModelLoader.load_embeddings()`, `FAISS.from_texts()`, `FAISS.load_local()`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `load_or_create(texts, metadatas)` | Optional lists | `FAISS` | Loads existing index or creates new one |
| `add_documents(docs: List[Document])` | Documents | `int` | Adds new docs with deduplication, returns count added |
| `_fingerprint(text, md)` | Text + metadata | `str` | Generates unique ID for deduplication |
| `_exists()` | None | `bool` | Checks if index files exist |

---

### 5. `/chat/query` Endpoint Flow

#### 5.1 ConversationalRAG

- **Location**: `src/document_chat/retrieval.py`
- **Purpose**: LCEL-based Conversational RAG with lazy retriever initialization
- **Inputs**:
  - `session_id: Optional[str]`
  - `retriever` - Optional pre-built retriever
- **Output**: Instance with LLM, prompts, and LCEL chain
- **Logic**: Builds LCEL pipeline: question rewriter → retriever → QA chain. Supports chat history for context-aware question reformulation.
- **Calls**: `ModelLoader.load_llm()`, `ModelLoader.load_embeddings()`, `PROMPT_REGISTRY`, `FAISS.load_local()`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `load_retriever_from_faiss(index_path, k, index_name)` | Path + params | `Retriever` | Loads FAISS index and builds LCEL chain |
| `invoke(user_input, chat_history)` | Question + history | `str` | Invokes full RAG pipeline, returns answer |
| `_build_lcel_chain()` | None | None | Constructs LCEL graph internally |
| `_format_docs(docs)` | `List[Document]` | `str` | Joins doc contents for prompt |
...
- **Purpose**: Central loader for embedding models and LLMs based on config
- **Inputs**: None (reads from config.yaml and environment)
- **Output**: Instance with `config`, `api_key_mgr`
- **Logic**: Loads .env in local mode, initializes ApiKeyManager, reads YAML config
- **Calls**: `ApiKeyManager()`, `load_config()`, `GoogleGenerativeAIEmbeddings`, `HuggingFaceEmbeddings`, `ChatGoogleGenerativeAI`, `ChatGroq`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `load_embeddings()` | None | `Embeddings` | Returns Google or HuggingFace embeddings based on config |
| `load_llm()` | None | `BaseChatModel` | Returns Google Gemini or Groq LLM based on env `LLM_PROVIDER` |
...
- **Location**: `utils/model_loader.py`
- **Purpose**: Manages API keys from environment variables or JSON secret
- **Inputs**: None (reads environment)
- **Output**: Instance with `api_keys` dict
- **Logic**: First tries to parse `API_KEYS` env as JSON (for ECS secrets), then falls back to individual env vars (`GROQ_API_KEY`, `GOOGLE_API_KEY`)
- **Calls**: `os.getenv()`, `json.loads()`

**Key Methods:**

| Method | Input | Output | Description |
|--------|-------|--------|-------------|
| `get(key: str)` | Key name | `str` | Returns API key value |
...
- **Location**: `utils/file_io.py`
- **Purpose**: File I/O utilities for session management and file uploads

| Function | Input | Output | Description |
|----------|-------|--------|-------------|
| `generate_session_id(prefix)` | Optional prefix | `str` | Creates timestamped UUID session ID |
| `save_uploaded_files(files, target_dir)` | Iterable + Path | `List[Path]` | Saves uploaded files with sanitized names |

#### 8.2 document_ops.py

- **Location**: `utils/document_ops.py`
- **Purpose**: Document loading and text concatenation utilities

| Function | Input | Output | Description |
|----------|-------|--------|-------------|
| `load_documents(paths)` | `Iterable[Path]` | `List[Document]` | Loads docs using PyPDFLoader, Docx2txtLoader, TextLoader |
| `concat_for_analysis(docs)` | `List[Document]` | `str` | Combines docs with source markers |
| `concat_for_comparison(ref, act)` | Two doc lists | `str` | Creates labeled reference/actual sections |
| `read_pdf_via_handler(handler, path)` | DocHandler + path | `str` | Reads PDF via handler's read method |

**FastAPIFileAdapter Class:**
- Adapts FastAPI `UploadFile` to provide `.name` and `.getbuffer()` interface for compatibility with handlers

#### 8.3 config_loader.py

- **Location**: `utils/config_loader.py`
- **Purpose**: Loads YAML configuration with flexible path resolution

| Function | Input | Output | Description |
|----------|-------|--------|-------------|
| `load_config(config_path)` | Optional path | `dict` | Loads config from yaml file |
| `_project_root()` | None | `Path` | Returns project root directory |
...
- **Purpose**: Central registry of LangChain prompt templates
- **Registry Keys**:
  - `document_analysis` - Metadata extraction prompt
  - `document_comparison` - Page-wise diff prompt
  - `contextualize_question` - Chat history question rewriter
  - `context_qa` - RAG answer generation prompt
  - `python_file_explanation` - Code documentation prompt

---

### 10. Models (Pydantic)

- **Location**: `model/models.py`
- **Purpose**: Pydantic models for structured LLM outputs

| Model | Fields | Description |
|-------|--------|-------------|
| `Metadata` | Summary, Title, Author, DateCreated, etc. | Document analysis response schema |
| `ChangeFormat` | Page, Changes | Single page comparison entry |
| `SummaryResponse` | `RootModel[list[ChangeFormat]]` | Full comparison response |
| `PromptType` | Enum values | Prompt registry key enum |

---

### 11. Logger

- **Location**: `logger/custom_logger.py`, `logger/__init__.py`
- **Purpose**: JSON-structured logging using `structlog`
- **Features**:
  - Timestamped log files in `logs/` directory
  - Console + file output
  - ISO timestamp, log level, JSON format
- **Global Instance**: `GLOBAL_LOGGER` (used across all modules)

---

### 12. Exception Handling

- **Location**: `exception/custom_exception.py`
- **Purpose**: Custom exception with detailed traceback information

| Class | Fields | Description |
|-------|--------|-------------|
| `DocumentPortalException` | `file_name`, `lineno`, `error_message`, `traceback_str` | Captures error location and full traceback |
...
| **LangChain** | LLM orchestration, document loading, text splitting, chains |
...
| **FAISS (faiss-cpu)** | Vector similarity search for document retrieval |
...
The **Document Portal** is a FastAPI-based application providing three core document processing capabilities:

1. **Document Analysis** (`/analyze`): Upload a PDF → extract structured metadata (title, author, summary, sentiment) using LLM with JSON output parsing.

2. **Document Comparison** (`/compare`): Upload two PDFs (reference + actual) → combine text → LLM performs page-wise diff → returns DataFrame of changes.

3. **Document Chat** (`/chat/index` + `/chat/query`): Upload documents → chunk → embed → store in FAISS → query using Conversational RAG with chat history support.

**Key Architecture Patterns:**
- **Session-based organization**: All operations use timestamped session IDs for file isolation
- **Lazy initialization**: RAG chain built only when retriever is loaded
- **Deduplication**: FaissManager tracks ingested documents via fingerprinting
- **LCEL pipelines**: Modern LangChain Expression Language for chain composition
- **Structured outputs**: Pydantic models + OutputFixingParser ensure reliable LLM responses
- **Environment flexibility**: Supports local (.env) and production (ECS secrets) API key management
